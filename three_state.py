...
class Three_state_CA(object):
    ...
    #__init__ is known as the constructor which simply allows you to define the attributes of the instantiated object from a given class that you want to pass as paramters
    # Here we want that Three_state_CA class have attributes rule number and an initial condition to be passed as parameters as that is all you need to know for the evolution of the cellular automata
    
    def __init__(self, rule_number, initial_condition):
        '''
        Initializes the simulator for the given rule number and initial condition.
        
        Parameters: These are as attributes too. We just desire for these attributes to be passed as parameters.
        ----------
        rule_number: int
            Integer value between 0 and 3**9-1, inclusive. The total number of possible rules are 3**9 for
            the three state CA case.
            
        initial_condition: list
            String used as the initial condition for the three state CA. Elements of the list should be 0s,1s 
            and 2s. 
        ...
        current_configuration: array_like
            List of the spatial configuration of the three state CA at the current time
        '''
        ...
        self._length = len(initial_condition) #the underscore before _length defines a private variable

    def evolve(self, time_steps):  #functions within a class is known as methods. Evolve is a method of this class
        '''
